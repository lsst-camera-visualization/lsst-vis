...
cmds = {
    ...
    help: function(state, args) {
        ...
        for (cmd in this.cmds) {
            state.term.echo(cmd + ": " + this.cmd.help_string);
        }
    },
    hot_pixel: function(data, cmd_args) {
        var help_string = 'find and plot hot pixels';
        var plotid = 'ffview';
    ...
    average_pixel: function(state, args) {
        ...
        if (!state.boxes[name]) {
            ...
            content = ['box', region.left, region.bottom, region.right - region.left, region.bottom - region.top, 0, '#color=red'].join(' ');
            console.log(content);
            ...
            firefly.getJsonFromTask("python", "average", {
                'rect': region
            }).then(function(data) {
                console.log(data);
